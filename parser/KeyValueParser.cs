...
namespace GUILib
{
	...
	/// <version>$Rev: 277 $, $Date: 2011-07-11 17:08:21 +0200 (Mo, 11 Jul 2011) $</version>
	public class KeyValueParser
	{
			...
			switch (symbols[this.curPosition])
			{
				...
				default:
					...
					if (symbols[this.curPosition].StartsWith("\""))
					{
						...
						read.content = this.ConvertControlSequences(read.content);
					}
					else
					{
						read.content = symbols[this.curPosition];
		...
		/// <summary>
		/// Converts control sequences written in style definitions to real control sequences unity understands. \\n will replaced with \n when read.
		/// </summary>
		/// <param name="content">
		/// String with encoded control sequences.
		/// </param>
		/// <returns>
		/// String with proper set control sequences.
		/// </returns>
		private string ConvertControlSequences(string content)
		{
