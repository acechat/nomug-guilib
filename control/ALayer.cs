...
{
	/// <summary>
	/// This is the abstract class from which all layers need to inherit.
	/// </summary>
	...
	public abstract class ALayer : IElementCollection, IKeyEvents
	{
		private int zIndex;
		...
		/// <summary>
		/// Modal layer constructor.
		/// </summary>
		/// <param name="modal">Whether the layer should be modal.</param>
		public ALayer(bool modal) : this()
		{
			if (modal)
			{
		...
		/// <summary>
		/// Whether the layer is modal. Modal layers will block interaction with other layers.
		/// </summary>
		public bool IsModal 
		{
		...
		/// <summary>
		/// The index of the layer on the pseudo z-axis.
		/// </summary>
		public int ZIndex 
		{
			...
			set
			{
				bool specialOverride = (this is AConsoleLayer);
				if (!this.IsModal || specialOverride)
				{
					if (value <= Configuration.MaxZIndex || specialOverride)
					{
						this.zIndex = value;
						LayerManager.Instance.SortLayers();
					}
					else
					{
						LayerManager.Instance.LogWarning("ZIndex of \"" + this.Name + "\" exceeds the maximum value of " + Configuration.MaxZIndex + ".");
					}
				}
				else
				{
					LayerManager.Instance.LogError("Can't set ZIndex of modal layer \"" + this.Name + "\".");
				}
			}
		}
		...
		/// <summary>
		/// Layers don't have a parent. Always returns null.
		/// </summary>
		public IElement Parent 
		{
			get { return null; }
			set { throw new Exception("Can't set parent of an layer"); }
		}
		...
		{
			get { return this.enabled && !(LayerManager.Instance.ModalLayer != null && LayerManager.Instance.ModalLayer != this); }
		}
		...
		public bool IsVisible 
		{
			get {
				if (!this.visible)
					return false;
				foreach (ALayer layer in LayerManager.Instance.Layers)
				{
					if (layer == this)
						return true;
				}
		...
		{
			if (element is ALayer) {
				LayerManager.Instance.LogError("Adding failed. Layer can't be a child.");
				return false;
			}
			if (element.Parent != null) {
				LayerManager.Instance.LogWarning("Adding failed. " + element.Name + " is already present in a collection. Try cloning it.");
				return false;
			}
		...
		public bool InsertElement(int index, IElement element)
		{
			if (element == null)
			{
				LayerManager.Instance.LogError("Insertion failed. Given element is null.");
				return false;
			}
			
			if (element is ALayer)
			{
				LayerManager.Instance.LogError("Insertion failed. Layer can't be a child.");
				return false;
			}
			
			if (element.Parent != null)
			{
				LayerManager.Instance.LogWarning("Insertion failed. " + element.Name + " is already present in a collection. Try cloning it.");
				return false;
			}
			
			if (index < 0 || index > this.children.Count)
			{
				LayerManager.Instance.LogWarning("Insertion of '" + element.Name + "' failed. Index is out of range.");
				return false;
			}
		...
		public bool RemoveElement(IElement element) 
		{
			...
			{
				LayerManager.Instance.LogWarning("Removal failed. The element " + element.Name + " is not a child of " + this.Name + ".");
			}
			return false;
		}
