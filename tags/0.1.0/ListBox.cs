...
namespace GUILib
{
	...
	/// <version>$Rev: 271 $, $Date: 2011-07-01 21:58:10 +0200 (Fr, 01 Jul 2011) $</version>
	public class ListBox : AListElement, ICloneable
	{
		...
		public override void DrawGUI()
		{
			...
			for (int i = 0; i < this.items.Count; i++)
			{
				...
				{
					drawStyle = new GUIStyle(this.itemStyle);
				}
				
				GUI.Button(new Rect(0, drawTop, drawWidth, this.itemHeight), this.items[i], drawStyle);
			}
			
			this.slider.Visible = (this.items.Count * this.itemHeight > this.Height);
			if(this.slider.Visible)
			{
				// FIXME workaround for slider not being part of the draw hierarchy
				this.slider.Enabled = this.IsEnabled;
				
				this.slider.Height = innerRect.height;
				this.slider.Left = innerRect.width - this.slider.Width;
