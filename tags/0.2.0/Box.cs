	...
	public class Box : AElement, IElementCollection, IDragEvents, ICloneable
	{
			...
			: base(source)
		{
			...
			foreach (IElement elem in source.children)
			{
				...
				{
					LayerManager.Instance.LogError("The element '" + elem.GetType().ToString() + "' doesn't implement ICloneable. '" + source.Name + "' couldn't be cloned completely.");
				}
			}
		}
		...
		public bool InsertElement(int index, IElement element)
		{
			...
			while (toCheck != null)
			{
				...
				{
					// circle found.
					LayerManager.Instance.LogError("Inserting " + element.Name + " to " + this.Name + " would create a circle. Aborting.");
					return false;
				}
		...
		public bool RemoveElement(IElement element)
		{
			...
			{
				LayerManager.Instance.LogWarning(this.Name + " does not contain " + element.Name + " so it can not be removed.");
				return false;
			}
		}
