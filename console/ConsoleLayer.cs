...
/// <summary>
/// This layer displays all messages that the console receives.
/// </summary>
...
public class ConsoleLayer : AConsoleLayer
{
	/// <summary>
	...
	/// <summary>
	/// Construct the ConsoleLayer 
	/// </summary>
	public ConsoleLayer(ConsoleData consoleData) : base()
	{
		...
		this.editbox.Name = "EditBox ConsolveLayer";
	...
	{
//		bool update = false;
//		string[] loggerNames = LayerManager.Instance.Console.Loggers();
//		Logger[] tempLoggers = new Logger[loggerNames.Length];
//		for (int i = 0; i < loggerNames.Length; i++)
//		{
//			tempLoggers[i] = LayerManager.Instance.Console.GetLogger(loggerNames[i]);
//		}
	}
	...
	private bool handleKeyEvent(IElement src, KeyCode key)
	{
		...
		{
			LayerManager.Instance.Console.Print("]] " + this.editbox.Text);
			if (!GUILib.ConsoleCommandRegistry.Instance.ExecuteCommand(this.editbox.Text))
			{
				...
				if (firstSpace > -1)
				{
					...
					LayerManager.Instance.Console.Print("Unknown command: \"" + command + "\"");
				}
			}
		}
		else
	...
	private void handleCommandSuggestionClick(IElement src)
	{
		...
		LayerManager.Instance.SetFocus(this.editbox);
	}
